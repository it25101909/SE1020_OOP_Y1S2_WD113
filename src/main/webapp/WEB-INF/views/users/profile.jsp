...
<head>
    ...
    <style>
        ...
        [data-theme="light"] .logout-btn-nav {
            background: rgba(239, 68, 68, 0.1);
            color: #c53030;
        ...
        h2 { color: white; text-align: center; margin-bottom: 30px; margin-top: 0; font-weight: 700; font-size: 28px; }
        ...
        .danger-zone h3 { color: #fca5a5; margin-top: 0; margin-bottom: 10px; font-weight: 700; text-align: center; }
        .danger-zone p { font-size: 14px; color: #fca5a5; margin-bottom: 20px; text-align: center; }
        .del-btn { 
            background: rgba(239, 68, 68, 0.1); 
            color: #fca5a5; 
            border: 1px solid rgba(239, 68, 68, 0.3); 
...
<body>
    ...
    <div class="profile-container" style="padding-top: 0; overflow: hidden;">
        <div style="height: 150px; background: url('https://images.unsplash.com/photo-1511367461989-f85a21fda167?auto=format&fit=crop&q=80&w=1000') center/cover; margin: 0 -40px 0 -40px; position: relative;">
            <img src="https://ui-avatars.com/api/?name=${loggedInUser.name}&background=f97316&color=fff&size=100" alt="Avatar" style="position: absolute; bottom: -50px; left: 50%; transform: translateX(-50%); border-radius: 50%; border: 4px solid var(--bg-dark); box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
        </div>
        <h2 style="margin-top: 60px;">Update Your Profile</h2>

        <c:if test="${param.status == 'updated'}">
            <div class="success-msg" style="background: rgba(16, 185, 129, 0.1); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.3); padding: 12px; border-radius: 8px; text-align: center; margin-bottom: 20px; font-weight: 600;">
        ...
        <form action="${pageContext.request.contextPath}/update-profile" method="post">
            ...
            <c:if test="${loggedInUser.role == 'Driver'}">
                ...
                <input type="text" name="companyName" value="${loggedInUser.companyName}" placeholder="e.g. C-XXXX">
                <p style="font-size: 12px; color: var(--text-muted); margin-top: -15px; margin-bottom: 20px;">Enter a Company ID to receive private orders from that company.</p>
            </c:if>
    ...
    <c:if test="${loggedInUser.role == 'Passenger'}">
        <div class="profile-container" style="border-top: 5px solid #0ea5e9; animation-delay: 0.1s;">
            ...
            <c:if test="${param.status == 'payment_updated'}">
                <div class="success-msg" style="background: rgba(56, 189, 248, 0.1); color: #7dd3fc; border-color: rgba(56, 189, 248, 0.3); padding: 12px; border-radius: 8px; text-align: center; margin-bottom: 20px; font-weight: 600;">
            ...
            <form action="${pageContext.request.contextPath}/update-payment" method="post">
                ...
                <input type="text" name="cardNumber" value="${loggedInUser.cardNumber}" placeholder="XXXX XXXX XXXX XXXX" required>
                
                <label>Expiry Date</label>
                <input type="text" name="cardExpiry" value="${loggedInUser.cardExpiry}" placeholder="MM/YY" required>
    ...
    <div class="profile-container danger-zone" style="animation-delay: 0.2s;">
        ...
        <form action="${pageContext.request.contextPath}/delete-account" method="post" onsubmit="return confirm('Are you absolutely sure you want to delete your account?');">
